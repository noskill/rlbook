...
The objective often used in practice is closer to

\begin{equation}
\label{practical-objective}
J_{\mu}(\theta)=\mathbb{E}_{s \sim \mu, a \sim \pi_\theta} \log   \pi(\cdot | s) A_{\pi_\theta}(s, a)
\end{equation}

where 
\begin{equation}
\label{uniform-in-time-distribution}
\mu(s) = \frac{1}{H} \sum_{t=0}^{H-1} d_t^{\pi \theta}(s)
\end{equation}

This is a uniform-in-time visitation distribution, not the discounted visitation distribution.
